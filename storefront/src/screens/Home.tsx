...
export default function Home() {
  ...
  // Server-driven home layout. When it resolves with sections we render those;
  // otherwise we fall back to the hardcoded layout below.
  ...
  useEffect(() => {
    ...
    catalogApi
      .getHomeLayout(selectedStore?.id)
      .then((res) => {
        setLayout(res.data?.sections || []);
      })
      .catch(() => {
        // Graceful degradation — fall back to the hardcoded layout.
        setLayout([]);
      })
  ...
  const hasServerSections = layout.length > 0;

  // Show the loader until both the layout request and the fallback data settle,
  // so we never flash the fallback before the server layout resolves.
  if (layoutLoading || (loading && !hasServerSections)) {
    return (
      <Layout>
  ...
  // Only surface the hard error when we also have no server layout to render.
  if (error && !hasServerSections) {
    return (
      <Layout>
    ...
    <Layout>
      <div className="bg-background min-h-screen">
        ...
        {hasServerSections ? (
          ...
          <div className="px-4 py-4 pb-24 flex flex-col gap-8">
            {layout.map((section) => (
              <SectionRenderer key={section.id} section={section} />
            ))}
          </div>
        ) : (
          /* ── Fallback: original hardcoded layout (graceful degradation) ── */
